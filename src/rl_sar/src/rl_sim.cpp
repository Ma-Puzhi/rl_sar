...
#endif
{
    ...
    //从参数服务器中接收string类型变量，并且赋值给当前类中的参数
    //主要获取了两个参数：ros_namespace与robot_name
    ...
    // read params from yaml 读取机器人参数配置base.yaml，加载进入ModelParams params中
    ...
    // init torch 对torch的设置
    //关闭libtorch的自动求导，可以提高效率。因为自动求导是用于参数迭代更新的，即训练过程，这里是部署过程，不需要更新参数权重
    torch::autograd::GradMode::set_enabled(false);
    //控制 CPU 后端线程数，即在 CPU 上运行张量计算时最多使用 4 个线程。
    torch::set_num_threads(4);

    // init robot
#if defined(USE_ROS1)
    //创建了joint_publishers_commands的动态数组容器，数组数量为this->params.num_of_dofs，即机器人自由度数
    //该容器代表要发布的关节命令
